...
cm2d = cm2d("{}/{}".format(path, fname), 
            xlabel, 
            'computer per far memory access', 
            'AP', 20, 20, 22, 22, 'bold') 

list1 = [1200, 1600, 2000, 3000, 4000, 5000, 6000, 7000, 8000, 9000]
ln = []
cm = []
fs = []
am = []
...
for i in list1:
    cm[j] = round(float(cm[j]/ln[j]), 1)
    am[j] = round(float(am[j]/ln[j]), 1)
    fs[j] = round(float(fs[j]/ln[j]), 1)
    j = j + 1
