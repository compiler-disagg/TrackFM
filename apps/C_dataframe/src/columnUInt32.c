...
static int setSelInt8(uint32_t * data, int8_t * selection_column, int8_t  selection, size_t size, uint32_t * dst) {
    int j = 0;
    int i = 0;
    uint32_t dat;
    uint32_t select_val;
...
static int setSelInt32(uint32_t * data, int32_t * selection_column, int32_t  selection, size_t size, uint32_t * dst) {
    int j = 0;
    int i = 0;
    uint32_t dat;
    uint32_t select_val;
...
static const char*
AddDateTime(DataFrame_ColumnUInt32* self, long datetime)
{
   
    return NULL;
}
static uint32_t 
GetValue(DataFrame_ColumnUInt32* self, size_t index)
{
...
static bool iis_unique(uint32_t * seen_val, uint32_t new_val, size_t size) { 
...
static int setUnique(uint32_t * val, uint32_t * data, size_t size) {
...
static uint32_t*
GetUnique(DataFrame_ColumnUInt32* self)
{
     //__attribute__((annotate("local_malloc")))
    uint32_t * val = (uint32_t *) malloc(SELF->size * sizeof(uint32_t));
...
static uint32_t* 
GetBySelInt32(DataFrame_ColumnUInt32* self, int32_t * selection_column, int32_t  selection) {
...
static uint32_t* 
GetBySelInt8(DataFrame_ColumnUInt32* self, int8_t * selection_column, int8_t  selection) {
...
static size_t GetGE(uint32_t  selection, uint32_t * dst,  uint32_t * data, size_t size) {
    int i = 0;
    int j = 0;
...
static uint32_t* 
GetBySelGE(DataFrame_ColumnUInt32* self, uint32_t  selection) {
...
static void group_BY_col(uint32_t * data, int8_t * group_column, 
		uint32_t ** groupby, int64_t  * group_size, int8_t * groups, size_t size, size_t ngroups) { 
...
#if 1
static uint32_t** 
GetBySelInt8Group(DataFrame_ColumnUInt32* self, int8_t * group_column, int8_t  * groups, int8_t ngroups, uint32_t * copy_data) {

    uint32_t * data = copy_data;

    int i = 0;
//     __attribute__((annotate("local_malloc")))
    uint32_t ** groupby = (uint32_t **) malloc(ngroups * sizeof(uint32_t *));

   int64_t * group_size = (int64_t *) malloc(ngroups * sizeof(int64_t));

   for (i = 0; i < ngroups; i++) {
	groupby[i] = malloc(SELF->size * sizeof(uint32_t));
	group_size[i] = 0;
   }
...
static uint32_t* 
GetBySelString(DataFrame_ColumnUInt32* self, char ** selection, char * select_val) {

    //__attribute__((annotate("local_malloc")))
    uint32_t * val = (uint32_t *) malloc(SELF->col_size * sizeof(uint32_t));
...
static uint32_t* 
GetBySelStringUnique(DataFrame_ColumnUInt32* self, char ** selection_column, char * selection) {
...
static int setSelInt8Unique(uint32_t * val, uint32_t * data, int8_t * selection_column, int8_t selection, size_t size) {
...
static uint32_t* 
GetBySelInt8Unique(DataFrame_ColumnUInt32* self, int8_t * selection_column, int8_t selection) {
...
static uint32_t*
GetAll(DataFrame_ColumnUInt32* self)
{
    return SELF->data;
}

static void copy_col(uint32_t * dst, uint32_t * src, uint64_t size) {
	for (int i = 0; i < size; i++) {
		dst[i] = src[i]; 
	}
}
...
static uint32_t*
GetCopy(DataFrame_ColumnUInt32* self)
{
    int i = 0;
    uint32_t * copy = (uint32_t *) malloc(SELF->col_size * sizeof(uint32_t));
    uint32_t * data = SELF->data;
...
static int64_t * 
GetBySelInt8GroupSize(DataFrame_ColumnUInt32* self) {
...
static char*
SetName(DataFrame_ColumnUInt32* self, char* name)
{
    char* n;
...
static bool
HasValue(DataFrame_ColumnUInt32* self, size_t index)
{
    bool na = DataFrame_BitVector_Get(&SELF->na, index);
    return !na;
}
...
static uint32_t *
Get(DataFrame_ColumnUInt32* self, size_t index)
{
    return &SELF->data[index];
}

static uint8_t*
GetNAs(DataFrame_ColumnUInt32* self)
{
    return SELF->na.data;
}

static int
GetUniqueSize(DataFrame_ColumnUInt32* self)
{
    return SELF->unique_size;
}

static int
GetColSize(DataFrame_ColumnUInt32* self)
{
    return SELF->col_size;
}
static int
GetSelSize(DataFrame_ColumnUInt32* self)
{
...
DataFrame_ColumnUInt32* DataFrame_ColumnUInt32_New(int type)
{
