...
static void settings_init(void) {
    ...
    settings.inter = NULL;
    settings.maxbytes = 2 * 1024 * 1024 * 1024; /* default is 64MB */
...
conn *conn_new(const int sfd, const int init_state, const int event_flags,
    ...
    void * sp0 = malloc(sizeof(conn));
    memset(sp0, 0, sizeof(conn));
        if (!(c = (conn *)sp0)) {
    ...
    __attribute__((annotate("local_malloc")))
    void * sp = malloc(c->rsize);
        c->rbuf = (char *)sp;
    void * sp1 = malloc(c->wsize);
        c->wbuf = (char *)sp1;
    void * sp2 = malloc(sizeof(item *) * c->isize);
        c->ilist = (item **)sp2;
    void * sp3 = malloc(sizeof(char *) * c->suffixsize);
        c->suffixlist = (char **)sp3;
    __attribute__((annotate("local_malloc")))
    void * sp4 = malloc(sizeof(struct iovec) * c->iovsize);
        c->iov = (struct iovec *)sp4;
    __attribute__((annotate("local_malloc")))
    void * sp5 = malloc(sizeof(struct msghdr) * c->msgsize);
        c->msglist = (struct msghdr *)sp5;
...
static void conn_shrink(conn *c) {
    ...
    if (c->rsize > READ_BUFFER_HIGHWAT && c->rbytes < DATA_BUFFER_SIZE) {
        ...
        if (c->rcurr != c->rbuf)
            memcpy(c->rbuf, c->rcurr, (size_t)c->rbytes);
...
static void complete_nread(conn *c) {
    ...
    char * val = ITEM_data(it) + it->nbytes - 2;
    if (val[0] != '\r' || val[1] != '\n') {
  //  if (strcmp(ITEM_data(it) + it->nbytes - 2, "\r\n") != 0) {
...
static int transmit(conn *c) {
	...
	uint64_t og_ptrs;
	char  buf1[4096];
	if (m->msg_iov[0].iov_base)
		memcpy(buf1, m->msg_iov[0].iov_base, m->msg_iov[0].iov_len);

	og_ptrs = (uint64_t)m->msg_iov[0].iov_base;

	m->msg_iov[0].iov_base = (char *)buf1;

        res = sendmsg(c->sfd, m, 0);

	m->msg_iov[0].iov_base = (char *)og_ptrs;
        ...
        if (settings.verbose > 0)
            fprintf(stderr, "Trans mit 2101 Failed to write, and not due to blocking sfd %lx, m %lx", c->sfd, m);
...
static void drive_machine(conn *c) {
    ...
    while (!stop) {
        ...
        case conn_nread:
            ...
            if (settings.verbose > 0)
                fprintf(stderr, "drive machine 2215 Failed to read, and not due to blocking\n");
            conn_set_state(c, conn_closing);
            break;
        ...
        case conn_swallow:
            ...
            /* otherwise we have a real error, on which we close the connection */
            if (settings.verbose > 0)
                fprintf(stderr, "Line 2260 Failed to read, and not due to blocking\n");
            conn_set_state(c, conn_closing);
            break;
